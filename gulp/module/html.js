...
const REPLACE_THEME = themeConfig.hasOwnProperty(global.theme) && themeConfig[global.theme].html ? REPLACE_HTML.concat(themeConfig[global.theme].html) : REPLACE_HTML;
...
const fnHtmlInclude = function(file) {
    const target = typeof file === 'string' ?
                    file :
                    SOURCE_HTML + '**/*.html';
    let stream = gulp.src(target, { base: SOURCE_HTML });

    REPLACE_THEME.forEach((item, index) => {
        stream = stream.pipe(replace(item[0], item[1]));
    })

    return stream.pipe(fileInclude({
            prefix: '@@',
